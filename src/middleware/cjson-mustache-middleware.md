...
middlewareHandler cJSONMustacheMiddleware(char *viewsPath,
  ...
  char * (^loadTemplate)(char *) = ^(char *templateFile) {
    ...
    snprintf(templatePath, templatePathSize, "%s/%s", viewsPath, (char *)templateFile);
  ...
  return Block_copy(^(UNUSED request_t *req, response_t *res, void (^next)(),
    ...
    res->render = ^(void *templateName, void *data) {
      ...
      if (strstr(templateName, ".mustache")) {
        ...
        size_t tempalteFileSize = strlen(templateName) + strlen(".mustache") + 1;
        templateFile = malloc(tempalteFileSize);
        snprintf(templateFile, tempalteFileSize, "%s.mustache", (char *)templateName);
      }
