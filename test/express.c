...
void expressTests(tape_t *t) {
  t->test("core", ^(tape_t *t) {
    ...
    t->test("GET", ^(tape_t *t) {
      ...
      t->strEqual(
          "query string", t->get("/qs\?value1=123\\&value2=34%205"),
    ...
    t->test("Middleware", ^(tape_t *t) {
      ...
      t->strEqual("custom request middleware",
                  t->get("/base/nested/m-isolated"), "No super-params-router");
    ...
    t->test("Redirect", ^(tape_t *t) {
      t->strEqual("redirect", t->get("/redirect"),
    ...
    t->test("Router", ^(tape_t *t) {
      ...
      t->strEqual("route params", t->get("/base/one/123/two/345/567.jpg"),
