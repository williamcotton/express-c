...
void resourceTests(tape_t *t, const char *databaseUrl) {
  ...
  t->test("resource", ^(tape_t *t) {
    ...
    t->test("find", ^(tape_t *t) {
      ...
      t->ok("bad find request",
            t->fetch("/api/v1/teams/sdfsdkj", "GET", headers, NULL)->size > 0);
    });
    ...
    t->test("include", ^(tape_t *t) {
      ...
      t->test("all", ^(tape_t *t) {
        ...
        t->strEqual(
            "multiple deep nested related resources with a filter",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
                     "size,max_temp&filter[date][eq]=2022-03-08",
                     "GET", headers, NULL),
            "{\"data\": [{\"type\": \"notes\", \"id\": \"1\", "
            "\"attributes\": {\"title\": \"a\", \"date\": \"2022-03-08\"}, "
            "\"relationships\": {\"employees\": {\"data\": [{\"id\": \"1\", "
        ...
        t->strEqual(
            "multiple deep nested related resources with multiple nested "
            "filters",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
                     "size,max_temp&filter[meetings][max_size][gt]=6&filter["
                     "teams][name]=design",
                     "GET", headers, NULL),
            "{\"data\": [{\"type\": \"notes\", \"id\": \"1\", "
            "\"attributes\": {\"title\": \"a\", \"date\": \"2022-03-08\"}, "
        ...
        t->strEqual(
            "multiple deep nested related resources with nested sorting",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
                     "size,max_temp&sort=-employees.name",
                     "GET", headers, NULL),
            "{\"data\": [{\"type\": \"notes\", \"id\": \"1\", "
            "\"attributes\": {\"title\": \"a\", \"date\": \"2022-03-08\"}, "
            "\"relationships\": {\"employees\": {\"data\": [{\"id\": \"1\", "
        ...
        t->strEqual(
            "bad req 1 multiple deep nested related resources with nested "
            "stats",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
            ...
            "\"date\": \"2022-03-08\"}, \"relationships\": {\"employees\": "
            "{\"data\": [{\"id\": \"1\", \"type\": \"employees\"}]}}}], "
            "\"meta\": {\"meetings\": {\"max_temp\": {\"bogus\": \"2\"}}}, "
            "\"included\": [{\"type\": \"employees\", \"id\": \"2\", "
            "\"attributes\": {\"name\": \"Bob\", \"email\": "
            "\"bob@email.com\"}, \"relationships\": {\"teams\": {\"data\": "
            "[{\"id\": \"2\", \"type\": \"teams\"}]}, \"notes\": {\"meta\": "
            "{\"included\": false}}}}, {\"type\": \"employees\", \"id\": "
        ...
        t->strEqual(
            "bad req 2 multiple deep nested related resources with nested "
            "stats",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
        ...
        t->strEqual(
            "bad req 3 multiple deep nested related resources with nested "
            "stats",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
                     "size,max_temp&sort=-id,-employees.name&stats[bogus]",
                     "GET", headers, NULL),
            "{\"data\": [{\"type\": \"notes\", \"id\": \"6\", "
            "\"attributes\": {\"title\": \"c\", \"date\": \"2022-03-09\"}, "
            "\"relationships\": {\"employees\": {\"data\": [{\"id\": \"2\", "
        ...
        t->strEqual(
            "bad req 4 multiple deep nested related resources with nested "
            "stats",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
        ...
        t->strEqual(
            "bad req 5 multiple deep nested related resources with nested "
            "stats",
            t->fetch("/api/v1/"
                     "notes?include=employees.teams.meetings&"
                     "fields[meetings]=max_"
