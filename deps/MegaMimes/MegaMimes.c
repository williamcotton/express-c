...
static void splitFileParts(const char *path, char **filename, char **fileextension, char **fileparents)
{
  ...
  else
  {
    ...
    *fileparents = malloc(bytes + 1);
    ...
    const char *dotPos = strrchr(path, '.');
    if (dotPos < slashPos || dotPos == NULL)
    { // not an extension
      *fileextension = malloc(1);
      strcpy(*fileextension, "");
...
static bool matchesMimeType(const char *target, const char *mimetype)
{
  ...
  if (!splitMimeTypeParts(target, &targetFirstPart, &targetLastPart, &targetDetailsPart))
    return false;
                               ...
                               bool mimetype, bool reset)
{
  ...
  for (; MegaMimeTypes[position][0]; ++position)
  {
    ...
    else
    {
      ...
      if (strcmp(target, (MegaMimeTypes[position][EXTENSION_POS]) + 1) == 0)
      { // start comparing from . not *
        if (extension)
          *extension = MegaMimeTypes[position][EXTENSION_POS];
        if (name)
          *name = MegaMimeTypes[position][MIMENAME_POS];
