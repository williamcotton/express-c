...
static char *keyval(char *head, int sflags, enum comp *comp)
{
  ...
  while (car && (escaped || (k = getcomp(head, sflags)) == C_no))
  {
    if (escaped)
      ...
      escaped = ((sflags & Mustach_With_JsonPointer) ? car == '~' : car == '\\') && (getcomp(head + 1, sflags) != C_no);
    if (!escaped)
      *w++ = car;
    head++;
    car = *head;
...
static enum sel sel(struct wrap *w, const char *name)
{
  ...
  else
  {
    ...
    if (w->itf->sel(w->closure, key))
      result = S_ok;
    else if (key[0] == '*' && !key[1] && !value && !*copy && (w->flags & Mustach_With_ObjectIter) && w->itf->sel(w->closure, NULL))
    ...
    if (result == S_ok)
    {
      ...
      while (result == S_ok && key)
      {
        if (w->itf->subsel(w->closure, key))
          /* nothing */;
        else if (key[0] == '*' && !key[1] && !value && !*copy && (w->flags & Mustach_With_ObjectIter))
          result = S_objiter;
        else
          result = S_none;
        key = getkey(&copy, sflags);
...
static int get_partial_from_file(const char *name, struct mustach_sbuf *sbuf)
{
  ...
  /* compute file size */
  if (fseek(file, 0, SEEK_END) >= 0 && (pos = ftell(file)) >= 0 && fseek(file, 0, SEEK_SET) >= 0)
  {
    /* allocate value */
    s = (size_t)pos;
    buffer = malloc(s + 1);
